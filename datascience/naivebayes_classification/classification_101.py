...
unknown_data_points = [unknown_1, unknown_2, unknown_3, unknown_4]
...
#nb_nltk_in_memory = nltk.NaiveBayesClassifier.train(train_set)
#nb_nltk_in_memory.train(train_set)

# We'll uncomment these when we implement our own classifiers! :)
import classifier_solutions
nb_ours_in_memory = classifier_solutions.NaiveBayesClassifierInMemory()
nb_ours_in_memory.train(train_set)
# nb_ours_in_database = classifier_solutions.NaiveBayesClassifierInDatabase()
# nb_ours_in_database.train(train_set)

#raw_input("\n\nHit enter to continue...")
#Make guesses about our unknown projects:
def print_classifier_predictions(data_points, classifier):
    print "Classifier: ", classifier
    for data_point in data_points:
        #print "Prediction for", data_point, "is:", classifier.classify(feature_extracting_function(data_point))
        classifier.predict(data_point)

print "Predicting the class of unknown data points"
#print_classifier_predictions(unknown_data_points, nb_nltk_in_memory)

# We'll uncomment these when we implement our own classifiers! :)
print_classifier_predictions(unknown_data_points, nb_ours_in_memory)
#print_classifier_predictions(unknown_data_points, nb_ours_in_database)
...
test_set = train_set #No no no no no.  Except for illustration cases like in a skillshare script, then yes. :P  Also note that in the subsequent exercise we properly evaluate accuracy on a true test set that is not the train set.
...
#print "Accuracy: "+str(nltk.classify.accuracy(nb_nltk_in_memory, test_set))
...
#print str(nb_nltk_in_memory.show_most_informative_features(20))
